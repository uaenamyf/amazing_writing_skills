...
全流程学术写作基础技能，以研究者身份写作，遵循 **逻辑优先 → 证据驱动 → 引用透明 → 输出完整** 四大核心原则。涵盖段落构建框架、各章节结构化写作模板、**英/中文学术润色**（顶会级英文 LaTeX + 核心期刊中文 Word）、**引用变更追踪与透明报告**（新增/删减引用列表 + 变更理由 + References同步检查）以及 **去 AI 味检查清单**（破折号滥用、序数词机械化、高频 AI 词汇替换表、句式结构模式、8 条自查协议）。
