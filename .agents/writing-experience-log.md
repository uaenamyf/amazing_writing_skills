...
| 2026-06-27 | 用户反馈 | 输出物持久化规范：所有生成内容必须写入 md 文件 | 全部 |
...
### [2026-06-27] 输出物持久化规范：所有生成内容必须写入 md 文件

**类型**：用户反馈

**触发场景**：用户要求建立输出规范，避免内容仅停留在聊天窗口。

**问题描述**：此前无明确规范要求将生成内容持久化到文件。聊天窗口中的输出无法追溯、复用或归档。

**解决方法**：
1. 在 `AGENT.md` 新增「输出物持久化规范」章节
2. 同类内容追加到已有文件，异类内容新建独立 md 文件
3. 新建文件命名格式：`{任务类型}_{论文简称}_{日期}.md`
4. 自审清单新增 F 类（输出物持久化）检查项
5. Phase 4 工作流中增加写入 md 文件步骤

**预防措施**：每次输出前检查自审清单 F 类。
